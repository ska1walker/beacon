...
überlebt, die Datenbank nicht. Für aicrm heißt das: **alle Firmen,
Kontakte, Geschäfte und der gesamte Verlauf wären weg.**

Bis der Ausfuhrpfad steht, ist die Sicherung von Hand zu machen:

```bash
pg_dump -h <box> -U aicrm aicrm > aicrm-$(date +%F).sql
```

`permission.appData` steht bereits im Manifest, damit der Ablageort
vorhanden ist, wenn der eingebaute Weg kommt.
